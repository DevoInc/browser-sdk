...
function processGenericError(status, callbacks) {
  ...
  logger(`browser-sdk :: chunksParser :: processGenericError :: id = ${status.id} :: bufferString = ${status.bufferString}`);
...
function processErrorParsed(callbacks, status, errorParsed) {
  ...
  logger(`browser-sdk :: chunksParser :: processErrorParsed :: id = ${status.id} :: errorParsed = ${errorParsed.msg}`);
