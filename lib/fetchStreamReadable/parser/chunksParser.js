    ...
    while(lines.length > 0){
      try {
        ...
        if (isData(line)) {
          ...
          joinMsgsIfObjectHasStrings(line);
        ...
        if (error)  {
          logger(`browser-sdk :: chunksParser :: parse :: id = ${status.id} :: errorPared = ${status.state}:: errorMsg = ${error.msg} ::  bufferString = ${status.bufferString}`);
