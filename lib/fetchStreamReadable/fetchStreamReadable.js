...
async function processStream(reader, callbacks, isOk, statusCode, separator = '\r\n') {
  const status = {
    bufferString: '',
    remains: '',
...
async function processReader(reader, status, callbacks, textDecoder, separator) {
  try {
    let isDone = false;
    let isFirst = true;
    do {
...
async function parseChunk(
  ...
  return new Promise((resolve, reject) => {
    const timerId = setTimeout(() => {
      // When no more data needs to be consumed or terminal error, break the reading
      if (result.done || isErrorParsed(status)) {
        ...
        try {
          // console.table({bufferString: status.bufferString, remains: status.remains, value: textDecoder.decode(value)});
...
function callback(callback, formatter = (x) => x) {
  return function (event) {
    if (callback) callback(formatter(event));
  };
}
