...
class OboeRequest {
   ...
   * @param {Object} processParams An object with stream process opt-in parameters
   ...
   */
  stream(options, processParams, callbacks) {
    this._request = oboe({
    ...
    this._request.node(this._getNodes(processParams, callbacks));
   ...
   * @private
   */
  _getNodes(processParams, callbacks) {
    return {
      '!.msg': msg => {
        this._msg = msg;
      },
      '!.object.d[*]': data => this._readRow(processParams, data, callbacks.data),
   ...
   */
  _readRow(processParams, event, callback) {
    if (processParams.mapMetadata === false) {
